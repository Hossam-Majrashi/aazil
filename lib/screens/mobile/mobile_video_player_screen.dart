...
/// Mobile Multi-Video Sandboxed Playlist Player Screen
///
/// Features:
/// - Real hardware-accelerated video decoding and surface rendering via media_kit.
/// - System audio output routing.
/// - Swipe gestures to switch between playlist videos (swipe left -> next, swipe right -> prev).
/// - Compact mobile bottom control bar: Play/Pause with visible contrasting icon, Seek bar, Volume dialog, Repeat toggle, Next/Prev.
/// - Expandable Playlist bottom sheet modal with all items to jump to any video.
/// - Auto-advance when video finishes with fresh sandboxed execution.
class MobileVideoPlayerScreen extends StatefulWidget {
  ...
  const MobileVideoPlayerScreen({
    ...
    this.initialIndex = 0,
  }) : assert(videos.length > 0, 'Playlist cannot be empty');
...
class _MobileVideoPlayerScreenState extends State<MobileVideoPlayerScreen> {
  ...
  void _initializePlayer() {
    try {
      ...
      debugPrint('Mobile player initialization fallback: $e');
    }
  }
  ...
  void _showPlaylistBottomSheet(BuildContext context, ThemeData theme, AppLocalizations l10n) {
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Material(
                      ...
                      child: ListTile(
                        ...
                        subtitle: Text(video.formattedSize, style: const TextStyle(fontSize: 11)),
                        trailing: isSelected
                            ? Text(
                                'NOW PLAYING',
                                style: TextStyle(
  ...
  void _showVolumeDialog(BuildContext context, ThemeData theme, AppLocalizations l10n) {
    showDialog(
      context: context,
      builder: (ctx) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          children: [
            // Video Viewport with Swipe Gestures
            Expanded(
              child: GestureDetector(
                behavior: HitTestBehavior.opaque,
                onHorizontalDragEnd: (DragEndDetails details) {
                  final velocity = details.primaryVelocity ?? 0;
                  if (velocity < -200 && canNext) {
                    _advanceToNextVideo();
                  } else if (velocity > 200 && canPrev) {
                    _goToPreviousVideo();
                  }
                },
                child: Container(
            ...
            // Compact Mobile Bottom Controls Bar
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
              decoration: BoxDecoration(
  ...
  Widget _buildFallbackPreview(MediaItem video, ThemeData theme) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 4),
        Text(
          'Sandboxed Process PID: ${_sandboxStatus?.isolatedPid ?? 1} • MIME: ${_mediaMetadata['mime_type'] ?? video.mimeType}',
