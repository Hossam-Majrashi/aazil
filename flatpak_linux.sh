...
# Desktop entry with full modern FreeDesktop / Flatpak metadata
...
EOF
cp "$STAGE_DIR/files/share/applications/$FLATPAK_ID.desktop" "$STAGE_DIR/files/share/applications/$APP_NAME.desktop"
...
for res in 16x16 20x20 24x24 32x32 48x48 64x64 128x128 256x256 512x512; do
    ...
    if [ -f "assets/icon/export_${res}.png" ]; then
        ...
        cp "assets/icon/export_${res}.png" "$STAGE_DIR/files/share/icons/hicolor/$res/apps/$APP_NAME.png"
        ...
        cp "assets/icons/export_${res}.png" "$STAGE_DIR/files/share/icons/hicolor/$res/apps/$APP_NAME.png"
    fi
done

# Fallback 512 icon & pixmaps
ICON_SRC=""
if [ -f "assets/icon/export_512x512.png" ]; then
...
if [ -n "$ICON_SRC" ]; then
    ...
    cp "$ICON_SRC" "$STAGE_DIR/files/share/icons/hicolor/512x512/apps/$APP_NAME.png"
    mkdir -p "$STAGE_DIR/files/share/pixmaps"
    cp "$ICON_SRC" "$STAGE_DIR/files/share/pixmaps/$FLATPAK_ID.png"
    cp "$ICON_SRC" "$STAGE_DIR/files/share/pixmaps/$APP_NAME.png"
fi
